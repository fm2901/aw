...
<div class="mb-3">
    ...
    <input type="text" class="form-control" required  id="first_name" name="first_name" placeholder="Enter your business name" autofocus/>
...
<div class="mb-3">
    ...
    <input type="text" class="form-control" required  id="apt" name="apt" placeholder="Enter your suite, office, unit#"/>
...
<div class="mb-3">
    <label for="last_name" class="form-label">The transfer vihicle titles to</label>
    @foreach($vehicleTo as $v)
        <div class="form-check">
            <input type="radio" required  id="vehicle_to" name="vehicle_to" value="{{ $v->id }}"/>
            <label class="form-check-label" for="vehicle_to">{{ $v->name }}</label>
        </div>
    @endforeach
    <x-input-error :messages="$errors->get('last_name')" class="mt-2" />
</div>
